...
/**
 * When I have sharing like this, I can represent it by treating the customer either as a value or as a reference.
 *
 * With a value, the customer data is copied into each order;
 ...
 * I then have to find all the copies and update them all. If I miss one, I’ll get a troubling inconsistency in my data. In this case, it’s often worthwhile to change the copied data into a single reference. That way, any change is visible to all the customer’s orders.
 *
 *
 ...
 *
 *
 * 如果設計上就是一個 shared data, then use `reference`.
 */
